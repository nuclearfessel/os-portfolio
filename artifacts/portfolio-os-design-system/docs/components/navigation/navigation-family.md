...
```tsx
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator } from '@workspace/portfolio-os-design-system/components/ui/dropdown-menu';
...
```tsx
import { ContextMenu, ContextMenuTrigger, ContextMenuContent, ContextMenuItem } from '@workspace/portfolio-os-design-system/components/ui/context-menu';
...
For the Portfolio OS desktop pattern (custom-positioned), see `ContextMenuSurface` in `portfolio-os.tsx`.
...
```tsx
import { Menubar, MenubarMenu, MenubarTrigger, MenubarContent, MenubarItem } from '@workspace/portfolio-os-design-system/components/ui/menubar';
...
```tsx
import { NavigationMenu, NavigationMenuList, NavigationMenuItem, NavigationMenuTrigger, NavigationMenuContent } from '@workspace/portfolio-os-design-system/components/ui/navigation-menu';
...
```tsx
import { Breadcrumb, BreadcrumbList, BreadcrumbItem, BreadcrumbLink, BreadcrumbPage, BreadcrumbSeparator } from '@workspace/portfolio-os-design-system/components/ui/breadcrumb';
...
<Breadcrumb>
  <BreadcrumbList>
    ...
    <BreadcrumbSeparator />
    <BreadcrumbItem><BreadcrumbPage>Signal Operations Platform</BreadcrumbPage></BreadcrumbItem>
  </BreadcrumbList>
</Breadcrumb>
```
...
```tsx
import { Pagination, PaginationContent, PaginationItem, PaginationLink, PaginationPrevious, PaginationNext } from '@workspace/portfolio-os-design-system/components/ui/pagination';
...
```tsx
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@workspace/portfolio-os-design-system/components/ui/tabs';
...
```tsx
import { SidebarProvider, Sidebar, SidebarContent, SidebarGroup, SidebarGroupLabel, SidebarGroupContent, SidebarMenu, SidebarMenuItem, SidebarMenuButton, SidebarInset, SidebarTrigger } from '@workspace/portfolio-os-design-system/components/ui/sidebar';
...
Note: `Sidebar` is a general-purpose application sidebar. For the Portfolio OS settings sidebar (section nav inside a window), use `SettingsNavSection` / `SettingsNavItem` instead.

For any sidebar used as section navigation inside a resizable Portfolio OS window, the universal window rule applies: at a narrow window-container width, smoothly transform it into a horizontal sub-navigation toolbar directly below the window toolbar. Do not key this transformation to viewport width, hide the navigation, or place it over the content.
