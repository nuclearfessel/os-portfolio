...
const OsPortfolioDemo = lazyPage(() =>
  import('./demos/os-portfolio').then(({ OsPortfolioDemo }) => OsPortfolioDemo),
);
...
// ── Individual OS Portfolio primitive pages ──────────────────────────────────────
const OsPortfolioActionButtonDemo = lazyPage(() =>
  import('./demos/os-portfolio-action-button').then(({ OsPortfolioActionButtonDemo }) => OsPortfolioActionButtonDemo),
);
const OsPortfolioSectionLabelDemo = lazyPage(() =>
  import('./demos/os-portfolio-section-label').then(({ OsPortfolioSectionLabelDemo }) => OsPortfolioSectionLabelDemo),
);
const OsPortfolioStatusIndicatorDemo = lazyPage(() =>
  import('./demos/os-portfolio-status-indicator').then(({ OsPortfolioStatusIndicatorDemo }) => OsPortfolioStatusIndicatorDemo),
);
const OsPortfolioSurfaceDemo = lazyPage(() =>
  import('./demos/os-portfolio-surface').then(({ OsPortfolioSurfaceDemo }) => OsPortfolioSurfaceDemo),
);
const OsPortfolioProjectCardDemo = lazyPage(() =>
  import('./demos/os-portfolio-project-card').then(({ OsPortfolioProjectCardDemo }) => OsPortfolioProjectCardDemo),
);
const OsPortfolioWindowSurfaceDemo = lazyPage(() =>
  import('./demos/os-portfolio-window-surface').then(({ OsPortfolioWindowSurfaceDemo }) => OsPortfolioWindowSurfaceDemo),
);
const OsPortfolioDockItemDemo = lazyPage(() =>
  import('./demos/os-portfolio-dock-item').then(({ OsPortfolioDockItemDemo }) => OsPortfolioDockItemDemo),
);
const OsPortfolioDesktopLauncherDemo = lazyPage(() =>
  import('./demos/os-portfolio-desktop-launcher').then(({ OsPortfolioDesktopLauncherDemo }) => OsPortfolioDesktopLauncherDemo),
);
const OsPortfolioStickyNoteDemo = lazyPage(() =>
  import('./demos/os-portfolio-sticky-note').then(({ OsPortfolioStickyNoteDemo }) => OsPortfolioStickyNoteDemo),
);
...
const OsPortfolioPrimitivesPage = lazyPage(() =>
  import('./demos/os-portfolio-primitives').then(({ OsPortfolioPrimitivesPage }) => OsPortfolioPrimitivesPage),
);
...
export const PUBLIC_VISIBILITY_MAP: Record<string, PublicVisibility> = {
  ...
  'os-portfolio-pilot': false,
  'os-portfolio-settings': false,
  // ── OS Portfolio individual primitives — detail pages, public but not in sidebar ───
  // Deep-linkable via #page=<id>; reachable from the os-portfolio-primitives directory.
  'action-button': false,
  'section-label': false,
...
export const NAV_GROUPS: NavGroup[] = [
  {
    ...
    entries: [
      ...
      // ── Family overviews — hidden from sidebar; deep-linkable by hash ──────
      {
        ...
        Page: OsPortfolioDemo,
      },
      {
        id: 'os-portfolio-settings',
      ...
      },
      {
        ...
        Page: OsPortfolioDesktopLauncherDemo,
      },
      {
        ...
        Page: OsPortfolioStickyNoteDemo,
      },
      {
...
// ── OS Portfolio detail IDs — public pages not listed in sidebar/search ─────────────
// These 18 IDs are deep-linkable public detail pages reachable from the
// os-portfolio-primitives directory. They are excluded from sidebar and search but are
// NOT "internal" pages — the "Internal" banner must not appear for them.
export const OS_PORTFOLIO_DETAIL_IDS: ReadonlySet<string> = new Set([
  ...
  'os-portfolio-pilot', 'os-portfolio-settings',
]);
...
export const DOC_COVERAGE_MAP: Record<string, DocCoverage> = {
  // ── OS Portfolio unified directory ─────────────────────────────────────────────────
  'os-portfolio-primitives': 'interactive+canonical',
  // ── OS Portfolio family overview ───────────────────────────────────────────────────
  'os-portfolio-pilot': 'interactive+canonical',
  'os-portfolio-settings': 'interactive+canonical',
  // ── OS Portfolio individual primitives ─────────────────────────────────────────────
...
if (import.meta.env.DEV) {
  ...
  const missingDetailIds = EXPECTED_OS_PORTFOLIO_DETAIL_IDS.filter((id) => !allIds.has(id));
  if (missingDetailIds.length > 0) {
    console.error(
      `[design-system] os-portfolio-primitives directory: ${missingDetailIds.length} detail page(s) not registered in ALL_ENTRIES:\n` +
