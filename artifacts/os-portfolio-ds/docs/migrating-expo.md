...
- Rewrite color/theme imports to
  `@workspace/os-portfolio-ds/lib/native-theme` and
  `@workspace/os-portfolio-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/os-portfolio-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/os-portfolio-ds` and retains only
