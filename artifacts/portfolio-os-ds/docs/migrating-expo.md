...
- Rewrite color/theme imports to
  `@workspace/portfolio-os-ds/lib/native-theme` and
  `@workspace/portfolio-os-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/portfolio-os-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/portfolio-os-ds` and retains only
