...
- [ ] Surface Effects uses one full-width row per effect at `680px` and narrower, then three equal columns above that container width.
...
```tsx
import { WindowSurface, SectionLabel } from '@workspace/os-portfolio-ds/components/ui/os-portfolio';
import {
  SettingsNavSection, SettingsNavItem,
  SettingsSectionHeader, SettingsDivider,
  SettingsToggleRow, SettingsSliderGroup,
} from '@workspace/os-portfolio-ds/components/ui/settings';
