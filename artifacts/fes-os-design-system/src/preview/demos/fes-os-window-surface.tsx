import { WindowSurface, SectionLabel } from '../../components/ui/fes-os';
import { CanonicalSpec } from '../md-renderer';
import { mdFesOsWindowSurface } from '../docs-map';

export function FesOsWindowSurfaceDemo() {
  return (
    <div className="space-y-8">
      <div className="space-y-4">
        ...
        <WindowSurface className="overflow-hidden">
          <div className="flex items-center gap-2 border-b border-border bg-muted/30 px-4 py-3">
            ...
            <span className="font-mono text-xs text-muted-foreground">~/fes/about</span>
          </div>
          <div className="p-5">
      ...
      <div className="space-y-4">
        ...
        <WindowSurface className="max-w-xs overflow-hidden">
          <div className="border-b border-border px-3 py-2 font-mono text-xs text-muted-foreground">~/fes/terminal</div>
          <div className="bg-black p-4 font-mono text-xs text-green-400">
            <p>$ whoami</p>
            <p className="mt-1 text-muted-foreground">fes_naqvi</p>
          </div>
        </WindowSurface>
      </div>

      <CanonicalSpec md={mdFesOsWindowSurface} title="WindowSurface reference" />
    </div>
  );
}
