...
Preview page: `portfolio-os-settings` in the living style guide
...
```tsx
import { SettingsNavSection, SettingsNavItem } from '@workspace/portfolio-os-design-system/components/ui/settings';
...
```tsx
import { SettingsToggleRow } from '@workspace/portfolio-os-design-system/components/ui/settings';
...
```tsx
import { SettingsSliderGroup } from '@workspace/portfolio-os-design-system/components/ui/settings';
...
Only surfaces carrying the `.portfolio-surface-translucent` package class (defined in
...
```tsx
import { SettingsSegmentedChoice } from '@workspace/portfolio-os-design-system/components/ui/settings';
...
```tsx
import { SettingsColorPreset } from '@workspace/portfolio-os-design-system/components/ui/settings';
...
```tsx
import { SettingsDivider } from '@workspace/portfolio-os-design-system/components/ui/settings';
...
```tsx
import { SettingsSectionHeader } from '@workspace/portfolio-os-design-system/components/ui/settings';
...
| --- | --- | --- |
| `.portfolio-scrollbar-window` | Add to each window shell | Keeps tracks transparent and fades styled thumbs in while the window is hovered or contains focus |
| `data-always-scrollbars` | Set on shell when `alwaysShowScrollbars === true` | Keeps the same `.portfolio-scrollbar-window` thumbs visible without changing their style or adding a gray track |
| `.portfolio-surface-translucent` | Add to window/dock/menu elements | Applies `backdrop-filter: blur(12px)` and uses `--accessibility-transparency` for background alpha |
...
| `data-contrast="low"` | Set on `:root` when `contrastTheme === 'low'` | Re-maps **all** semantic channel vars (background, foreground, card, primary, accent, destructive, chart-1–5, sidebar-primary/accent/ring, border, input, ring) to `--contrast-*-hsl` values. Also forces `.portfolio-surface-translucent` opaque. No per-component override needed for package components. |
| `data-contrast="high"` | Set on `:root` when `contrastTheme === 'high'` | Re-maps all semantic channel vars to `--hc-*-hsl` values (black/white/yellow/cyan). Also forces `.portfolio-surface-translucent` opaque. 3px yellow `var(--hc-focus)` focus rings globally via `*:focus-visible`. |

**Scope of translucency suppression under contrast modes:** `[data-contrast]` forces only `.portfolio-surface-translucent` elements opaque. Inline `rgba()` backgrounds or Tailwind `bg-*/alpha` utilities set directly by the consuming app are not automatically removed — the app should avoid alpha backgrounds when a contrast theme is active.
