...
export function DesignSystemBrowser() {
  const [theme, setTheme] = useState<'light' | 'dark'>(() =>
    localStorage.getItem('portfolio-os-design-system.theme') === 'dark' ? 'dark' : 'light',
  );
  ...
  // public detail page (i.e. a Portfolio OS primitive or family overview that is
  // intentionally omitted from sidebar but is still a public-facing document).
  const isPublicDetailPage = PORTFOLIO_OS_DETAIL_IDS.has(active.id);
  const isPrimitiveDetailPage =
    isPublicDetailPage &&
    active.id !== 'portfolio-os-pilot' &&
    active.id !== 'portfolio-os-settings';
  const isHiddenPage =
  ...
  useEffect(() => {
    ...
    localStorage.setItem('portfolio-os-design-system.theme', theme);
  }, [theme]);
          ...
          <div className="mx-auto max-w-5xl">
            {isPrimitiveDetailPage && (
              <Button
                ...
                size="sm"
                className="mb-6"
                onClick={() => selectPage('portfolio-os-primitives')}
              >
                ← Back to Portfolio OS primitives
              </Button>
            )}
