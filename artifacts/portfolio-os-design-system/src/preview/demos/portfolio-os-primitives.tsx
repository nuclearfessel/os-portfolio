/**
 * Portfolio OS Primitives — unified directory page.
 *
 * Organises all 18 Portfolio OS + Settings primitives into a compact, scannable
...
import { Surface } from '../../components/ui/portfolio-os';
...
export const PORTFOLIO_OS_PRIMITIVE_IDS = [
...
if (import.meta.env.DEV) {
  ...
  const ids = new Set(PORTFOLIO_OS_PRIMITIVE_IDS as readonly string[]);
  const missing = expected18.filter((id) => !ids.has(id));
  if (missing.length > 0) {
    console.error(
      `[portfolio-os-primitives] Directory is missing ${missing.length} primitive(s):\n` +
      missing.map((id) => `  • ${id}`).join('\n'),
    );
  }
  if (PORTFOLIO_OS_PRIMITIVE_IDS.length !== 18) {
    console.error(
      `[portfolio-os-primitives] Expected 18 primitives, found ${PORTFOLIO_OS_PRIMITIVE_IDS.length}.`,
    );
  }
}
...
export function PortfolioOsPrimitivesPage() {
  return (
