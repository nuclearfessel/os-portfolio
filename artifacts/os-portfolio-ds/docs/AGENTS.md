...
workspace package (`@workspace/os-portfolio-ds`): other artifacts depend
...
`@workspace/os-portfolio-ds/components/...`. Never use a `@/` alias inside
...
for `@workspace/os-portfolio-ds` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/os-portfolio-ds` as a `workspace:*` dependency,
