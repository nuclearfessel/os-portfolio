...
```tsx
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem, DropdownMenuSeparator } from '@workspace/os-portfolio-ds/components/ui/dropdown-menu';
...
```tsx
import { ContextMenu, ContextMenuTrigger, ContextMenuContent, ContextMenuItem } from '@workspace/os-portfolio-ds/components/ui/context-menu';
...
For the OS Portfolio desktop pattern (custom-positioned), see `ContextMenuSurface` in `os-portfolio.tsx`.
...
```tsx
import { Menubar, MenubarMenu, MenubarTrigger, MenubarContent, MenubarItem } from '@workspace/os-portfolio-ds/components/ui/menubar';
...
```tsx
import { NavigationMenu, NavigationMenuList, NavigationMenuItem, NavigationMenuTrigger, NavigationMenuContent } from '@workspace/os-portfolio-ds/components/ui/navigation-menu';
...
```tsx
import { Breadcrumb, BreadcrumbList, BreadcrumbItem, BreadcrumbLink, BreadcrumbPage, BreadcrumbSeparator } from '@workspace/os-portfolio-ds/components/ui/breadcrumb';
...
```tsx
import { Pagination, PaginationContent, PaginationItem, PaginationLink, PaginationPrevious, PaginationNext } from '@workspace/os-portfolio-ds/components/ui/pagination';
...
```tsx
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@workspace/os-portfolio-ds/components/ui/tabs';
...
```tsx
import { SidebarProvider, Sidebar, SidebarContent, SidebarGroup, SidebarGroupLabel, SidebarGroupContent, SidebarMenu, SidebarMenuItem, SidebarMenuButton, SidebarInset, SidebarTrigger } from '@workspace/os-portfolio-ds/components/ui/sidebar';
...
Note: `Sidebar` is a general-purpose application sidebar. For the OS Portfolio settings sidebar (section nav inside a window), use `SettingsNavSection` / `SettingsNavItem` instead.

For any sidebar used as section navigation inside a resizable OS Portfolio window, the universal window rule applies: at a narrow window-container width, smoothly transform it into a horizontal sub-navigation toolbar directly below the window toolbar. Do not key this transformation to viewport width, hide the navigation, or place it over the content.
