...
**How to apply:** Restrict automatic release triggers to `main`, guard manual runs against non-main refs, and never pass prerelease flags. Build and validate both required ZIP assets, then attach both to the same release after approved work is merged to `main`.