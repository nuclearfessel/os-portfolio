/**
 * Settings UI primitives — Fes OS Design System
 *
...
export function SettingsNavSection({ className, label = 'Settings sections', children, ...props }: SettingsNavSectionProps) {
  return (
    <nav
      aria-label={label}
      className={classes(
        'fes-settings-nav flex flex-col gap-0.5 overflow-y-auto',
        className,
      )}
      {...props}
    >
...
export const SettingsNavItem = forwardRef<HTMLButtonElement, SettingsNavItemProps>(
  ({ className, active = false, icon, children, type = 'button', ...props }, ref) => (
    <button
      ...
      type={type}
      className={classes(
        'fes-settings-nav-item flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-sm transition-colors duration-100',
        'text-muted-foreground hover:bg-muted hover:text-foreground',
        'focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-ring',
        active && 'fes-settings-nav-item--active bg-primary/10 text-primary',
        className,
      )}
      {...props}
    >
      {icon && (
        <span className="fes-settings-nav-icon flex size-[18px] shrink-0 items-center justify-center" aria-hidden="true">
          {icon}
        </span>
      )}
      {children}
...
export function SettingsDivider({ className, ...props }: HTMLAttributes<HTMLHRElement>) {
  return (
    <hr
      className={classes('fes-settings-divider my-5 border-0 border-t border-border/50', className)}
      {...props}
    />
  );
}
...
export function SettingsSectionHeader({ label, description, className, ...props }: SettingsSectionHeaderProps) {
  return (
    <div className={classes('fes-settings-section-header grid gap-0.5', className)} {...props}>
      <span className="fes-settings-label text-[13px] font-medium text-foreground">{label}</span>
      {description && (
        <span className="fes-settings-description text-[11px] leading-relaxed text-muted-foreground">{description}</span>
      )}
    </div>
  );
}
...
export function SettingsToggleRow({
  ...
  return (
    <label
      htmlFor={id}
      className={classes(
        'fes-settings-toggle-row flex cursor-pointer items-center gap-3.5 rounded-lg border border-border/30 bg-card/40 px-3 py-2.5 transition-colors duration-100',
        'hover:border-border/60 hover:bg-card/60',
        disabled && 'fes-settings-toggle-row--disabled pointer-events-none opacity-45',
        className,
      )}
      data-testid={testId}
    >
      <div className="fes-settings-toggle-label-group flex min-w-0 flex-1 flex-col gap-0.5">
        <span className="fes-settings-label text-[13px] font-medium text-foreground">{label}</span>
        {description && (
          <span className="fes-settings-description text-[11px] leading-relaxed text-muted-foreground">{description}</span>
        )}
      ...
      <button
        ...
        aria-label={label}
        className={classes(
          'fes-settings-toggle-switch relative h-5 w-9 shrink-0 appearance-none rounded-full transition-colors duration-150',
          'bg-border/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1',
          checked && 'fes-settings-toggle-switch--on bg-primary',
        )}
      ...
      >
        ...
        <span
          className={classes(
            'fes-settings-toggle-knob pointer-events-none absolute left-[3px] top-[3px] size-3.5 rounded-full bg-white shadow-sm transition-transform',
...
export function SettingsSliderGroup({
  ...
  return (
    <div
      className={classes(
        'fes-settings-slider-group grid gap-2 rounded-lg border border-border/30 bg-card/40 p-3',
        className,
      )}
      data-testid={testId}
    >
      {/* Heading row: label + live value output */}
      <div className="fes-settings-slider-heading flex items-center justify-between gap-3">
        <span className="fes-settings-label text-[13px] font-medium text-foreground" id={`${id}-label`}>
          {label}
        </span>
        <output
          htmlFor={id}
          aria-live="polite"
          className="fes-settings-slider-value font-mono text-[11px] text-primary"
      ...
      {(guidanceStart || guidanceEnd) && (
        <div
          className="fes-settings-slider-guidance flex justify-between font-mono text-[9px] text-muted-foreground"
          aria-hidden="true"
        >
...
export function SettingsSegmentedChoice({
  ...
  return (
    <div
      ...
      className={classes('fes-settings-segmented-choice flex flex-wrap gap-1.5', className)}
      data-testid={testId}
    >
      {options.map((option) => {
        ...
        return (
          <button
            ...
            className={classes(
              'fes-settings-segment-chip inline-flex items-center gap-1 rounded-full border px-3 py-1 text-[11px] font-medium transition-colors duration-100',
              'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1',
              isSelected
                ? 'fes-settings-segment-chip--selected border-primary bg-primary text-primary-foreground'
...
export function SettingsContrastCard({
  ...
  return (
    <button
      ...
      data-testid={testId}
      className={classes(
        'fes-settings-contrast-card flex min-w-24 max-w-[148px] flex-1 flex-col items-start gap-1.5 rounded-md p-0 text-left outline-none',
    ...
    >
      ...
      <div
        className={classes(
          'fes-settings-contrast-preview w-full overflow-hidden rounded-md border-2 transition-colors duration-100',
          'aspect-[3/2] flex flex-col',
          selected
            ? 'border-primary'
            : 'border-border/40 hover:border-primary/50',
          variant === 'standard' && 'fes-settings-contrast-preview--standard',
          variant === 'low' && 'fes-settings-contrast-preview--low',
          variant === 'high' && 'fes-settings-contrast-preview--high',
        )}
        aria-hidden="true"
      ...
      <span
        className={classes(
          'fes-settings-contrast-label inline-flex items-center gap-1 text-[11px] font-semibold leading-none',
      ...
      {description && (
        <span className="fes-settings-contrast-desc pb-0.5 text-[10px] leading-snug text-muted-foreground/70">
          {description}
        </span>
      )}
    </button>
...
export function SettingsColorPreset({
  ...
  return (
    <button
      ...
      data-testid={testId}
      className={classes(
        'fes-settings-color-preset grid cursor-pointer justify-items-start gap-1 border-0 bg-transparent p-0 text-left text-[10px] text-muted-foreground',
    ...
    >
      <span
        className={classes(
          'fes-settings-color-preset-swatch grid size-12 place-items-center rounded-md border shadow-md transition-all duration-100',
          selected
            ? 'fes-settings-color-preset-swatch--selected border-2 border-primary shadow-[0_0_0_2px_color-mix(in_srgb,var(--color-primary)_18%,transparent)]'
            : 'border-border/40',
        )}
